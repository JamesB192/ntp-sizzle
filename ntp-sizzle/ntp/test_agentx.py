...
class TestMIBControl(unittest.TestCase):
    ...
    def test_addNode(self):
        ...
        c.addNode((0,), 1, 2)
        self.assertEqual(
            c.oidTree, {0: {"reader": 1, "writer": 2, "subids": None}}
        )
    ...
    def test_getOID_andNext(self):
        ...
        self.assertEqual(c.getOID(AP.OID((0, 1))), (None, None, None))
        # Test empty, with generator
        self.assertEqual(
            c.getOID(AP.OID((0, 1)), True), (None, None, None, None)
        )
        # Test existent, no generator
        self.assertEqual(
            c.getOID(AP.OID((0, 2, 1))), (AP.OID((0, 2, 1)), 20, 21)
        )
        ...
        # Test non-existent, no generator
        self.assertEqual(
            c.getNextOID(AP.OID((0, 0))), (AP.OID((0, 2, 0)), 10, 11)
        )
        # Test empty, no generator
        self.assertEqual(
            c.getNextOID(AP.OID((0, 1))), (AP.OID((0, 2, 0)), 10, 11)
        )
        # Test empty, with generator
        ...
        # Test existent, no generator
        self.assertEqual(
            c.getNextOID(AP.OID((0, 2, 1))), (AP.OID((0, 2, 2)), 30, 31)
        )
        # Test existent, with generator
    ...
    def test_getOIDsInRange(self):
        ...
        self.assertEqual(
            c.getOIDsInRange(rng),
            [(AP.OID((0, 2, 1)), 20, 21), (AP.OID((0, 2, 2)), 30, 31)],
        )
...
class TestPacketControl(unittest.TestCase):
    ...
    def test__doloop(self):
        ...
        try:
            ...
            respPkt = AP.ResponsePDU(
                True, 13, 0, 0, 0, AP.RSPERR_NOT_OPEN, 0
            )
    ...
    def test_initNewSession(self):
        ...
        self.assertEqual(
            openpkt,
            AP.OpenPDU(True, 23, 0, 0, 30, (), "NTPsec SNMP subagent"),
        )
        self.assertEqual(
            registerpkt,
            AP.RegisterPDU(
    ...
    def test_waitForResponse(self):
        ...
        self.assertEqual(
            pkts, [AP.ResponsePDU(True, 100, 200, 300, 0, 0, 0)]
        )
        self.assertEqual(
            ret, AP.ResponsePDU(True, 42, 23, 100, 0, 0, 0)
        )
        ...
        self.assertEqual(
            pkts, [AP.ResponsePDU(True, 42, 23, 100, 0, 0, 0)]
        )
    ...
    def test_checkResponses(self):
        ...
        try:
            ...
            self.assertEqual(
                p.packetLog, {(42, 24, 200): (50, "bar", callback)}
            )
        finally:
            AX.time = timetemp
    ...
    def test_sendPacket(self):
        ...
        self.assertEqual(
            p.packetLog, {(0, 1, 2): (30.0, testpkt, "foo")}
        )
    ...
    def test_handleGetPDU(self):
        readReturns = [1]
        read = lambda oid: AP.Varbind(
            AP.VALUE_INTEGER, oid, readReturns.pop(0)
        )
        mjig = AX.MIBControl()
        ...
        respPkt = AP.ResponsePDU(
            ...
            0,
            0,
            (AP.Varbind(AP.VALUE_NO_SUCH_OBJECT, (1, 0)),),
        )
        self.assertEqual(sentPkt, respPkt)
        # Test non-existent
        sock.data = []
    ...
    def test_handleGetNextPDU(self):
        ...
        # Test successful
        pkt = AP.GetNextPDU(
            True, 42, 23, 100, [AP.SearchRange((0, 0), ())]
        )
        ...
        sock.data = []
        pkt = AP.GetNextPDU(
            True, 42, 23, 100, [AP.SearchRange((1, 0), ())]
        )
        p.handle_GetNextPDU(pkt)
        self.assertEqual(len(sock.data), 1)
        sentPkt = AP.decode_packet(sock.data[0])[0]
        respPkt = AP.ResponsePDU(
            ...
            0,
            0,
            (AP.Varbind(AP.VALUE_END_OF_MIB_VIEW, (1, 0)),),
        )
        self.assertEqual(sentPkt, respPkt)
    ...
    def test_handle_TestSetPDU(self):
        ...
        pkt = AP.TestSetPDU(
            True, 42, 23, 100, [AP.Varbind(AP.VALUE_INTEGER, (0,), 16)]
        )
        ...
        self.assertEqual(
            mjig.setVarbinds, [AP.Varbind(AP.VALUE_INTEGER, (0,), 16)]
        )
        self.assertEqual(mjig.setHandlers, [write])
        self.assertEqual(
            mjig.setUndoData, [AP.Varbind(AP.VALUE_INTEGER, (0,), 10)]
        )
        ...
        writeCalls = []
        pkt = AP.TestSetPDU(
            True, 42, 23, 100, [AP.Varbind(AP.VALUE_INTEGER, (1,), 16)]
        )
        p.handle_TestSetPDU(pkt)
        self.assertEqual(len(sock.data), 1)
        sentPkt = AP.decode_packet(sock.data[0])[0]
    ...
    def test_handle_CommitSetPDU(self):
        ...
        handler = lambda a, vb: (
            handlerCalls.append((a, vb)),
            handlerReturns.pop(0),
        )[1]
        mjig = AX.MIBControl()
        ...
        self.assertEqual(sentPkt, respPkt)
        self.assertEqual(
            handlerCalls, [("commit", "foo"), ("commit", "bar")]
        )
        # Test failure
        sock.data = []
        handlerCalls = []
    ...
    def test_handle_UndoSetPDU(self):
        handlerCalls = []
        handlerReturns = [AP.ERR_NOERROR, AP.ERR_NOERROR]
        handler = lambda a, vb, ud: (
            handlerCalls.append((a, vb, ud)),
            handlerReturns.pop(0),
        )[1]
        mjig = AX.MIBControl()
        mjig.setVarbinds = ["foo", "bar"]
...
class TestAgentx(unittest.TestCase):
    def test_walkMIBTree(self):
        ...
        submaker = lambda: {
            0: {"reader": None, "writer": None, "subids": None},
            1: {
                "reader": None,
                "writer": None,
                "subids": {
            ...
            2: {"reader": None, "writer": None, "subids": None},
        }
