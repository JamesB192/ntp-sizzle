    ...
    def test_lfp(self):
        for in_string, to_float, to_string in self.lfp_set:
            ...
            self.assertAlmostEqual(
                ntp.ntpc.lfptofloat(in_string), to_float
            )

    def test_nul_trunc16b(self):
        k_type = "aes128"
        key = ntp.util.hexstr2octets("0fd2287c1e97a50cb9d3cb9f80debcb6")
        ...
        mac2 = ntp.ntpc.mac(
            ntp.poly.polybytes(sample[:48]),
            ntp.poly.polybytes(key),
            k_type,
        )
        mac1 = ntp.poly.polybytes(sample[52:])
        self.assertEqual(
            [len(mac1), mac1], [len(mac2), mac2], msg="nul trunc"
        )
